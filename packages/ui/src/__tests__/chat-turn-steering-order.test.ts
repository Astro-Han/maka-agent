...
import { reconcileLiveTurnBuffer } from '../live-turn-buffer.js';
...
test('keeps post-steering work below the steering row even when it continues the same step', () => {
  ...
  live = applyLiveTurnEvent(live, {
    type: 'tool_start', id: 'e5', turnId: 'turn-1', stepId: 'm1', toolUseId: 'tool-2', toolName: 'Shell', args: {}, ts: 6,
  });
  live = applyLiveTurnEvent(live, {
...
test('keeps confirmed slice positions after releasing the live buffer, but never retains changed facts', () => {
  const projection = createTranscriptProjection();
  let live: LiveTurnProjection | undefined = applyLiveTurnEvent(armLiveTurn('turn'), {
    type: 'text_delta', id: 'delta-1', turnId: 'turn', messageId: 'answer', ts: 1, text: 'before',
  });
  live = applyLiveTurnEvent(live, {
    type: 'steering_message', id: 'steering', turnId: 'turn', messageId: 'instruction', ts: 2, content: { text: 'steer' },
  });
  live = applyLiveTurnEvent(live, {
    type: 'text_delta', id: 'delta-2', turnId: 'turn', messageId: 'answer', ts: 3, text: 'after',
  });
  live = applyLiveTurnEvent(live, {
    type: 'text_complete', id: 'text-end', turnId: 'turn', messageId: 'answer', ts: 4, text: 'beforeafter',
  });
  const expected = ['text:before', 'user:steer', 'text:after'];
  assert.deepEqual(timelineOrder(projection.project({ locale: 'en', messages: [], liveTurns: [live] })[0]!.timeline), expected);
  const messages: StoredMessage[] = [
    { type: 'user', id: 'root', turnId: 'turn', ts: 0, text: 'question' },
    { type: 'user', id: 'instruction', turnId: 'turn', ts: 2, text: 'steer', steeringEventId: 'steering' },
    { type: 'assistant', id: 'answer', turnId: 'turn', ts: 4, text: 'beforeafter', modelId: 'test' },
    { type: 'turn_state', id: 'end', turnId: 'turn', ts: 5, status: 'completed' },
  ];
  const buffer = reconcileLiveTurnBuffer([live], messages);
  assert.equal(buffer, undefined);
  const settled = projection.project({ locale: 'en', messages, liveTurns: buffer });
  assert.deepEqual(timelineOrder(settled[0]!.timeline), expected);
  assert.ok(settled[0]!.timeline.every((item) => !('live' in item) || !item.live));
  assert.equal(projection.project({ locale: 'en', messages: structuredClone(messages) }), settled);
  const corrected = structuredClone(messages);
  (corrected[2] as Extract<StoredMessage, { type: 'assistant' }>).text = 'corrected';
  assert.deepEqual(timelineOrder(projection.project({ locale: 'en', messages: corrected })[0]!.timeline), ['user:steer', 'text:corrected']);
  assert.deepEqual(projection.project({ sessionId: 'another', locale: 'en', messages: [] }), []);
});

test('steering does not reset whole-stream text processing', () => {
  for (const kind of ['text', 'thinking'] as const) {
    let live: LiveTurnProjection | undefined = applyLiveTurnEvent(armLiveTurn('turn'), {
      type: `${kind}_delta`, id: 'prefix', turnId: 'turn', messageId: 'm', ts: 1, text: 'Authorization: Bearer secret-prefix',
    });
    live = applyLiveTurnEvent(live, {
      type: 'steering_message', id: 'steering', turnId: 'turn', messageId: 'instruction', ts: 2, content: { text: 'steer' },
    });
    live = applyLiveTurnEvent(live, {
      type: kind === 'text' ? 'thinking_delta' : 'text_delta', id: 'interleaved', turnId: 'turn', messageId: 'm', ts: 3, text: 'other content',
    });
    live = applyLiveTurnEvent(live, {
      type: `${kind}_delta`, id: 'suffix', turnId: 'turn', messageId: 'm', ts: 4, text: '-SECRET_SUFFIX tail',
    });
    const displayed = () => live!.steps.map((step) => step[kind]?.text ?? '').join('');
    assert.equal(displayed(), 'Authorization: Bearer <redacted> tail');
    live = applyLiveTurnEvent(live, {
      type: `${kind}_complete`, id: 'end', turnId: 'turn', messageId: 'm', ts: 5, text: 'Authorization: Bearer secret-prefix-SECRET_SUFFIX tail',
    });
    assert.equal(displayed(), 'Authorization: Bearer <redacted> tail');
  }
});
