...
describe('shared chat input behavior', () => {
  ...
  it('ranks a name match above a description match', () => {
    ...
    assert.equal(mentionMatchRank('compact', 'side'), 2);
    assert.equal(mentionMatchRank('debug', 'package:entry', 'Debug workspace'), 0);
    assert.equal(mentionMatchRank('debug', 'inspect', 'Inspect', 'debug'), 0);
  });
