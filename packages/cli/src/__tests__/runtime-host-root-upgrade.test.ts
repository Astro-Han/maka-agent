...
test('successor CLI selects before takeover and retains its target after activation failure', async (t) => {
  ...
  const current: RuntimeHostManagedDeploymentConfig = {
    ...
    listeners: { localIpc: true, websocket: { host: '127.0.0.1', port: 0, path: '/runtime-host' } },
  ...
  assert.deepEqual(selection.outcome, { kind: 'manual_action', reason: 'compatibility_mismatch' });
