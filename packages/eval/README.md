...
The Maka subject launches the native CLI (`config.executable`, or `maka` on PATH) in a fresh State Root. Its required `connection` contains a public plugin `provider` identity, opaque `configuration`, and `authentication: { method, inputEnvironment }`. The named environment variable must be a declared credential and contain the provider's JSON authentication input, for example `{"apiKey":"..."}` for `api-key`. The bundled examples use `MAKA_AUTH_INPUT`. The shim authenticates with a stable attempt ID before selecting the model; it never writes raw provider credentials.
