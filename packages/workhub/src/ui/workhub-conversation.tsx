...
export function WorkHubDelegationStatus(props: {
  ...
  return (
    <Text
      type="supporting"
      color="secondary"
      className="workhub-delegation-status"
      role="status"
      data-work-state={state}
    >
      {props.showName ? `${work.targetSessionName}: ` : ''}
      {stateLabel}
    </Text>
  );
}

export function WorkHubConversation(
  props: ComponentProps<typeof ChatView> & {
    workLinks: readonly WorkHubLinkedWork[];
    onOpenWork(sessionId: string): void;
    promptStates?: ReadonlyMap<string, WorkHubDelegationState>;
  },
) {
  ...
  const workByTurn = useMemo(
    () =>
      new Map(
        [...worksByTurn].flatMap(([turnId, works]) =>
          works.length === 1 ? [[turnId, works[0]!.targetSessionId] as const] : [],
        ),
      ),
    [worksByTurn],
  );
  const promptRailDecorations = useMemo(
    () =>
      new Map(
        [...workByTurn].map(([turnId, sessionId]) => [
          turnId,
          {
  ...
  const promptTextByTurn = new Map(
    chat.messages?.flatMap((message) =>
      message.type === 'user' ? [[message.turnId, message.text.slice(0, 80)] as const] : [],
    ),
  );
  const turnDecorations = new Map(
    [...worksByTurn].map(([turnId, works]) => [
      turnId,
      {
        ...
        promptStatus: (
          <>
            {works.map((work, index) => (
              <span key={work.id}>
                {index > 0 ? ' / ' : ''}
                <WorkHubDelegationStatus work={work} locale={locale} showName={works.length > 1} />
              </span>
            ))}
          </>
        ),
        messageRail:
          works.length === 1 ? (
            <Button
              ...
              className="workhub-message-rail"
              ...
              onClick={() =>
                highlight.toggleWork({
                  sessionId: works[0]!.targetSessionId,
                  name: works[0]!.targetSessionName,
                })
              }
            />
          ) : undefined,
        header: (
          <div className="workhub-turn-heading">
            {works.map((work) => (
              <Link
                ...
                style={
                  {
                    '--workhub-work-hue': workHubIdentityHue(work.targetSessionId),
                  } as CSSProperties
                }
              ...
              >
                {work.workspaceName
                  ? `${work.workspaceName} / ${work.targetSessionName}`
                  : work.targetSessionName}
              </Link>
            ))}
          </div>
        ),
  ...
  for (const [turnId, state] of promptStates ?? []) {
    if (!turnDecorations.has(turnId))
      turnDecorations.set(turnId, {
        header: <></>,
        accentColor: undefined,
        messageRail: undefined,
        promptStatus: (
          <WorkHubDelegationStatus
            locale={locale}
            work={{
              id: turnId,
              coordinationTurnId: turnId,
              targetSessionId: '',
              targetSessionName: '',
              state,
            }}
  ...
  const matchingTurns = new Set(
    [...worksByTurn]
      .filter(([, works]) => works.some((work) => work.targetSessionId === selected?.sessionId))
      .map(([turnId]) => turnId),
  );
  const messages = selected
    ? chat.messages.filter(
        (message) => message.turnId !== undefined && matchingTurns.has(message.turnId),
      )
    : chat.messages;
  const liveTurns = selected
    ? chat.liveTurns?.filter((turn) => matchingTurns.has(turn.turnId))
    : chat.liveTurns;
  const activeTurn =
    selected && chat.activeTurn && !matchingTurns.has(chat.activeTurn.turnId)
      ? undefined
      : chat.activeTurn;
  const navigationTurn = [...chat.messages]
    .reverse()
    .find(
      (message) =>
        message.turnId &&
        worksByTurn
          .get(message.turnId)
          ?.some((work) => work.targetSessionId === highlight.navigationWork?.sessionId),
    )?.turnId;
  return (
    <>
      {selected && (
        <div
          className="workhub-conversation-filter"
          role="region"
          aria-label={copy.filterConversation}
        >
          <Text type="supporting">{selected.name}</Text>
          <Button
            variant="ghost"
            label={copy.clearConversationFilter}
            onClick={() => highlight.selectWork(undefined)}
          />
        </div>
      )}
      <ChatView
        {...chat}
        scrollTargetTurn={
          navigationTurn && highlight.navigationWork
            ? { turnId: navigationTurn, nonce: highlight.navigationWork.nonce, preserveFocus: true }
            : chat.scrollTargetTurn
        }
        messages={messages}
        liveTurns={liveTurns}
        transientMessages={
          selected
            ? chat.transientMessages?.filter(
                (message) => message.hostTurnId && matchingTurns.has(message.hostTurnId),
              )
            : chat.transientMessages
        }
        ...
        onPromptRailHighlight={(turnId) =>
          highlight.highlight(turnId ? workByTurn.get(turnId) : undefined)
        }
      />
    </>
  );
