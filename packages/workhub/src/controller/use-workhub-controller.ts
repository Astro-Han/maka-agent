...
} from '@maka/ui/session-execution';
...
interface SendAttempt {
  ...
  receipt?: import('@maka-agent/plugin-sdk/host').ExecutionReceipt;
  admission: 'pending' | 'unknown' | 'admitted' | 'terminal' | 'rejected';
  reconciling?: 'reading' | 'again';
...
/** Owned by the conversation shell across Client replacement, never execution authority. */
export interface WorkHubContinuation {
  answer?: { sessionId: string; input: WorkHubAnswerInput; stop?: boolean };
...
export function useWorkHubController(
  ...
  services: CoordinationSessionServices,
  onSubmit?: () => void,
  ...
  const [sessions, setSessions] = useState<
    Awaited<ReturnType<CoordinationSessionServices['listSessions']>>
  >([]);
  ...
  const [{ transientMessages, messageQueue }, setMessagePresentation] =
    useState<MessagePresentation>({
      transientMessages: [],
      messageQueue: { entries: [] },
    });
  const [execution, setExecution] = useState<SessionExecutionProjection>();
  const executionRef = useRef<SessionExecutionProjection>(undefined);
  const [liveTurns, setLiveTurns] = useState<LiveTurnBuffer>();
  const liveTurn =
    liveTurns?.find((turn) => turn.turnId === execution?.rootTurn?.turnId) ?? liveTurns?.at(-1);
  ...
  const [turnStates, setTurnStates] = useState<
    Record<string, import('../model/linked-work.js').WorkHubDelegationState>
  >({});
  ...
  const pendingQueued = useRef<QueuedAttempt>(undefined);
  const ownedAnswer = useRef<WorkHubContinuation['answer']>(undefined);
  const forgetAnswer = (operationId: string) => {
    if (
      continuation?.answer === ownedAnswer.current &&
      continuation?.answer?.input.operationId === operationId
    )
      continuation.answer = undefined;
  };
  const forgetQueued = (messageId: string) => {
    if (continuation?.queued?.messageId === messageId) continuation.queued = undefined;
  };
    ...
    if (currentSessionId.current === target && retracted.length > 0) {
      ...
      setMessagePresentation((previous) => ({
        ...
        messageQueue: {
          ...
          entries: previous.messageQueue.entries.filter((entry) => !ids.has(entry.messageId)),
        },
      }));
      if (pendingQueued.current?.sessionId === target && ids.has(pendingQueued.current.messageId)) {
        forgetQueued(pendingQueued.current.messageId);
  ...
  async function deliverStop(attempt: SendAttempt): Promise<void> {
    if (
      ...
      currentSessionId.current !== attempt.sessionId
    )
      return;
    if (attempt.stop !== 'requested') {
      attempt.stop = 'resend';
      return;
    }
    ...
    let failed = false;
    try {
      const target = attempt.receipt.invocation.turn_id;
      if (executionRef.current?.rootTurn?.turnId === target)
        await stopTurn(attempt.sessionId, target);
      else await services.cancelAnswer(attempt.sessionId, attempt.input);
      attempt.stop = undefined;
      forgetAnswer(attempt.input.operationId);
    } catch (reason) {
      failed = true;
  ...
  function reconcileAdmission(target: string, turnId: string, terminal = false) {
    ...
    if (
      !attempt ||
      attempt.sessionId !== target ||
      attempt.receipt?.invocation.turn_id !== turnId ||
      attempt.admission === 'rejected'
    )
      return;
    if (attempt.admission === 'unknown' && currentSessionId.current === target) setError(undefined);
    if (terminal || !attempt.stop) forgetAnswer(attempt.input.operationId);
    if (terminal) {
  ...
  function acceptAnswer(attempt: SendAttempt, result: WorkHubAnswerResult): boolean {
    if (pendingSend.current !== attempt) return true;
    ...
    const turnId = result.receipt.invocation.turn_id;
    const observed = executionRef.current?.rootTurn;
    const terminal =
      result.progress?.state === 'ended' ||
      (observed?.turnId === turnId &&
        ['completed', 'failed', 'cancelled'].includes(observed.status)) ||
      transcriptRef.current.messages.some(
        (message) =>
          message.turnId === turnId &&
          message.type === 'turn_state' &&
          message.status !== 'running',
      );
    reconcileAdmission(attempt.sessionId, turnId, terminal);
    if (current) {
      setError(undefined);
      setMessagePresentation((previous) => ({
        ...previous,
        transientMessages: previous.transientMessages.flatMap((message) => {
          if (message.id !== attempt.input.operationId) return [message];
          if (
            transcriptRef.current.messages.some(
              (row) => row.type === 'user' && row.id === result.receipt.messageId,
            )
          )
            return [];
          return [{ ...message, id: result.receipt.messageId, hostTurnId: turnId }];
        }),
      }));
      if (terminal) refreshSessions.current();
      setLiveTurns((previous) => {
        const observed = previous?.filter((turn) => turn.turnId !== attempt.input.operationId);
        if (attempt.admission === 'terminal') return observed;
        return reconcileLiveTurnBuffer(
          observed?.some((turn) => turn.turnId === turnId)
            ? observed
            : retainLiveTurn(observed, armLiveTurn(turnId)),
  ...
  async function recoverSend(): Promise<void> {
    ...
    if (
      !attempt ||
      attempt.sessionId !== currentSessionId.current ||
      attempt.admission !== 'unknown'
    )
      return;
    if (attempt.reconciling) {
      attempt.reconciling = 'again';
      return;
    }
    attempt.reconciling = 'reading';
    try {
      ...
      const again = (attempt.reconciling as SendAttempt['reconciling']) === 'again';
      attempt.reconciling = undefined;
      if (again) void recoverSend();
    }
  }
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      void Promise.all([
        ...
        sessionId ? services.getSession(sessionId) : undefined,
      ])
        .then(([next, coordination]) => {
          if (!disposed && read === revision) {
            setSessions((current) => {
              ...
              return [
                ...next,
                known && known.revision > coordination.revision ? known : coordination,
              ];
            });
            for (const turnId of coordination?.runningTurnIds ?? [])
              reconcileAdmission(sessionId!, turnId);
          }
        })
  ...
  useEffect(() => {
    // Each instance owns its in-flight flags. A retired instance's late reply
    // cannot clear a successor submission with a different identity.
    const retained = continuation?.answer;
    if (!pendingSend.current && retained && retained.sessionId === sessionId) {
      ownedAnswer.current = { ...retained };
      continuation.answer = ownedAnswer.current;
      pendingSend.current = {
        ...retained,
        admission: 'unknown',
        stop: retained.stop ? 'requested' : undefined,
      };
    }
    if (
      !pendingQueued.current &&
      continuation?.queued &&
      continuation.queued.sessionId === sessionId
    )
      pendingQueued.current = { ...continuation.queued };
    setChoices([]);
    ...
    setLiveTurns(pending ? [armLiveTurn(pending.input.operationId)] : undefined);
    setStopPending(Boolean(pending?.stop));
    const queued =
      pendingQueued.current &&
      pendingQueued.current.sessionId === sessionId &&
      !pendingQueued.current.observed
        ? pendingQueued.current
        : undefined;
    if (queued) setError(workHubLiveCopy[localeRef.current].sendUnknown);
    setMessagePresentation({
      ...
      transientMessages: [
        ...(pending
          ? [
              {
                id: pending.input.operationId,
                hostTurnId: pending.input.operationId,
        ...
        ...(queued
          ? [
              {
                id: queued.messageId,
                hostTurnId: queued.turnId,
                text: queued.text,
                attachments: queued.attachments,
                ts: Date.now(),
                transientPlacement: queued.placement,
                pendingSteering: queued.placement === 'current_turn',
              },
            ]
          : []),
      ],
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      void services
        .listActiveInteractions(sessionId)
        .then((requests) => {
          if (!disposed && interactionRevision.current === readRevision)
            setInteractions((current) => reconcileInteractions(current, sessionId, requests));
        })
        .catch((reason: unknown) => {
          if (!disposed) report(reason);
        });
    ...
    return () => {
      disposed = true;
      unsubscribe();
      if (refreshInteractions.current === refresh) refreshInteractions.current = () => {};
    };
  }, [services, sessionId]);
  ...
  useEffect(() => {
    ...
    let modelRevision = 0;
    const refreshModels = () => {
      const read = ++modelRevision;
      void services
        .modelChoices(sessionId)
        .then((next) => {
          if (!disposed && read === modelRevision) setChoices(next);
        })
        .catch((reason: unknown) => {
          if (!disposed && read === modelRevision) report(reason);
        });
    };
    ...
    const unsubscribe = services.observe(
      ...
      (event) => {
        ...
        interactionRevision.current++;
        const terminal =
          event.type === 'complete' || event.type === 'abort' || event.type === 'error';
        setInteractions((current) =>
          terminal
            ? clearInteractions(current, sessionId)
        ...
        if (event.type === 'queue_update') {
          ...
          if (pendingQueued.current && ids.has(pendingQueued.current.messageId)) {
            pendingQueued.current.observed = true;
            forgetQueued(pendingQueued.current.messageId);
          }
          setMessagePresentation((previous) => ({
            messageQueue: {
              entries: entries.filter((entry) => entry.state === 'queued'),
        ...
        if (event.type === 'message_admission') {
          if (event.outcome === 'admitted') {
            if (pendingQueued.current?.messageId === event.messageId) {
              pendingQueued.current.observed = true;
              forgetQueued(event.messageId);
            }
            ...
            setMessagePresentation((previous) => {
              const queued = previous.messageQueue.entries.find(
                (entry) => entry.messageId === event.messageId,
              );
              const local = previous.transientMessages.find(
                (message) => message.id === event.messageId,
              );
              const content = queued
                ? {
                    ...queued.content,
                    text: queued.content.displayText ?? queued.content.text,
                    id: queued.messageId,
                    ts: local?.ts ?? event.ts,
                  }
                : local;
              const messages = previous.transientMessages.filter(
                (message) => message.id !== event.messageId,
              );
              if (
                content &&
                !transcriptRef.current.messages.some(
                  (message) => message.type === 'user' && message.id === event.messageId,
                )
              ) {
                messages.push({
                  ...content,
              ...
              return {
                messageQueue: {
                  ...previous.messageQueue,
                  entries: previous.messageQueue.entries.filter(
                    (entry) => entry.messageId !== event.messageId,
                  ),
                },
            ...
            setMessagePresentation((previous) => ({
              ...
              transientMessages: previous.transientMessages.filter(
                (message) => message.id !== event.messageId,
              ),
            }));
          }
        ...
        reconcileAdmission(
          sessionId,
          event.turnId,
          event.type === 'abort' || event.type === 'error' || event.type === 'complete',
        );
        setLiveTurns((previous) => {
      ...
      (projection) => {
        if (!disposed) {
          executionRef.current = projection;
          setExecution(projection);
        }
      },
    );
    const opening = services.openTranscript(
      sessionId,
      (snapshot) => {
        if (disposed) return;
        const attempt = pendingSend.current;
        if (attempt?.sessionId === sessionId && attempt.receipt) {
          const messages = snapshot.messages.filter(
            (message) => message.turnId === attempt.receipt?.invocation.turn_id,
          );
          if (messages.length)
            reconcileAdmission(
              sessionId,
              attempt.receipt.invocation.turn_id,
              messages.some(
        ...
        if (
          ...
          snapshot.messages.some(
            (message) => message.type === 'user' && message.id === queued.messageId,
          )
        ) {
          queued.observed = true;
          forgetQueued(queued.messageId);
        }
              ...
              !snapshot.messages.some(
                (message) =>
                  ...
                  (message.id === pending.id ||
                    (pending.id === pending.hostTurnId && message.turnId === pending.hostTurnId)),
              ),
          ),
        }));
        const settled = snapshot.messages.filter(
          (message) =>
        ...
        setLiveTurns((previous) => {
          ...
          for (const message of settled)
            if (next) next = settleLiveTurnBufferStep(next, message.id);
  ...
  const pendingAdmission =
    !!attempt &&
    attempt.sessionId === sessionId &&
    (attempt.admission === 'pending' || attempt.admission === 'unknown');
  const runningTurnId =
    activeHostTurn(execution)?.turnId ??
    (attempt && attempt.sessionId === sessionId && attempt.admission === 'admitted'
      ? attempt.receipt?.invocation.turn_id
      : undefined);
  const busy = sending || pendingAdmission || Boolean(runningTurnId);
  async function send(text: string, attachments: AttachmentRef[], requestedMode?: FollowUpMode) {
    ...
    if (pendingAdmission) {
      if (attempt?.admission === 'unknown') void recoverSend();
      return false;
    }
    ...
    const sameQueued =
      previousQueued?.sessionId === target &&
      previousQueued.text === text &&
      JSON.stringify(previousQueued.attachments) === JSON.stringify(attachments)
        ? previousQueued
        : undefined;
    if (
      previousQueued?.sessionId === target &&
      !previousQueued.observed &&
      (!sameQueued || sameQueued.placement !== placement)
    ) {
      setError(
        workHubLiveCopy[localeRef.current][
          previousQueued.placement === 'current_turn' ? 'retrySteering' : 'retryFollowup'
        ],
      );
      return false;
    }
    ...
    try {
      if (queuedTurnId) {
        ...
        if (continuation) continuation.queued = { ...attempt };
        if (!attempt.observed)
          setMessagePresentation((previous) => ({
            ...previous,
            transientMessages: [
              ...previous.transientMessages.filter((message) => message.id !== attempt.messageId),
              {
                id: attempt.messageId,
                hostTurnId: queuedTurnId,
                text,
                attachments: [...attachments],
                ts: Date.now(),
                transientPlacement: attempt.placement,
                pendingSteering: attempt.placement === 'current_turn',
              },
            ],
          }));
        viewportNavigation.followLatest(target);
        const result = await services.enqueueMessage(
          target,
          attempt.messageId,
          text,
          attachments,
        ...
        if (result === 'rejected' && pendingQueued.current === attempt) {
          forgetQueued(attempt.messageId);
          pendingQueued.current = undefined;
          setMessagePresentation((previous) => ({
            ...previous,
            transientMessages: previous.transientMessages.filter(
              (message) => message.id !== attempt.messageId,
            ),
          }));
        }
        if (result !== 'admitted' && !attempt.observed)
          throw new Error(
            workHubLiveCopy[localeRef.current][
              result === 'unknown' ? 'sendUnknown' : 'sendNotAdmitted'
            ],
          );
        if (pendingQueued.current === attempt) {
          forgetQueued(attempt.messageId);
          pendingQueued.current = undefined;
        }
        return true;
      ...
      const sameRejected =
        previous?.sessionId === target &&
        previous.admission === 'rejected' &&
        previous.input.text === text &&
        JSON.stringify(previous.input.attachments ?? []) === JSON.stringify(attachments);
      const attempt: SendAttempt = {
        sessionId: target,
        input: {
          operationId: sameRejected ? previous.input.operationId : crypto.randomUUID(),
          text,
          ...(attachments.length ? { attachments: [...attachments] } : {}),
        },
        admission: 'pending',
      };
      const pendingRejectedTurnId =
        previous?.admission === 'rejected' ? previous.input.operationId : undefined;
      pendingSend.current = attempt;
      if (continuation) {
        ownedAnswer.current = { sessionId: target, input: attempt.input };
        continuation.answer = ownedAnswer.current;
      }
      setTurnStates((states) => ({ ...states, [attempt.input.operationId]: 'running' }));
      setLiveTurns((previous) => retainLiveTurn(previous, armLiveTurn(attempt.input.operationId)));
      setMessagePresentation((previous) => ({
        ...previous,
        transientMessages: [
          ...previous.transientMessages.filter(
            (message) =>
              message.hostTurnId !== attempt.input.operationId &&
              message.hostTurnId !== pendingRejectedTurnId,
          ),
          {
            id: attempt.input.operationId,
            hostTurnId: attempt.input.operationId,
            text,
            ts: Date.now(),
            attachments: [...attachments],
            transientPlacement: 'current_turn',
      ...
      if (currentSessionId.current === target) {
        const attempt = pendingSend.current;
        const failedTurnId = attempt?.input.operationId;
        ...
        if (failedTurnId && attempt?.admission === 'rejected')
          setTurnStates((states) => ({ ...states, [failedTurnId]: 'failed' }));
        setLiveTurns((previous) =>
          previous?.filter((turn) => turn.turnId !== failedTurnId || !turn.unconfirmed),
        );
        report(reason);
      }
      return false;
  ...
  async function stop() {
    if (!sessionId || (!runningTurnId && !pendingAdmission) || stopPending) return;
    setStopPending(true);
    const attempt = pendingSend.current;
    if (
      attempt?.sessionId === sessionId &&
      (pendingAdmission || attempt.receipt?.invocation.turn_id === runningTurnId) &&
      attempt.admission !== 'terminal' &&
      attempt.admission !== 'rejected'
    ) {
      attempt.stop = 'requested';
      if (continuation?.answer?.input.operationId === attempt.input.operationId)
        continuation.answer.stop = true;
      if (attempt.admission === 'unknown') await recoverSend();
      await deliverStop(attempt);
      return;
    }
    try {
      if (runningTurnId) await stopTurn(sessionId, runningTurnId);
    } catch (reason) {
      report(reason);
    } finally {
      setStopPending(false);
    ...
    try {
      const result = await services.configureModel(sessionId, {
        ...
        target: {
          ...
          model: {
            connection_id: input.llmConnectionId,
            connection_slug: input.llmConnectionSlug,
            model: input.model,
          },
        },
      });
  ...
  return {
    ...
    respondToUserForm: async (
      response: import('@maka/core/interaction').InteractionFormResponse,
    ) => {
    ...
    turnStates,
    pendingTurnId:
      pendingSend.current?.sessionId === sessionId
        ? pendingSend.current?.receipt?.invocation.turn_id
        : undefined,
    ...
    transientMessages,
    messageQueue,
    updateQueuedEntry: (entryId: string, revision: number, text: string) =>
      mutateQueue((target) => services.updateQueueEntry(target, entryId, revision, text)),
    deleteQueuedEntry: (entryId: string) =>
      mutateQueue((target) => services.retractQueueEntry(target, entryId)),
    promoteQueuedEntry: (entryId: string) =>
      mutateQueue((target) => services.promoteQueueEntry(target, entryId)),
    reorderQueuedEntries: (entryIds: readonly string[]) =>
      mutateQueue((target) => services.reorderQueueEntries(target, entryIds)),
    viewportNavigation,
    liveTurn,
    liveTurns,
    ...
    canRetry: Boolean(
      readError ||
        (error &&
          (pendingSend.current?.admission === 'unknown' ||
            pendingSend.current?.admission === 'rejected' ||
            (pendingQueued.current && !pendingQueued.current.observed))),
    ),
    send,
    stop,
    changeModel,
    ...
    retry: () => {
      ...
      } else if (pendingQueued.current && !pendingQueued.current.observed) {
        const queued = pendingQueued.current;
        void send(
          queued.text,
          queued.attachments,
          queued.placement === 'current_turn' ? 'steer' : undefined,
        );
      }
    },
