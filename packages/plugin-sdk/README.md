...
- `call.history.source({ sessionId, turnId, messageId })` reads one exact input before preparation, including accepted queue edits and owned attachment references. It is not an aggregate transcript row or proof of admission; resubmission needs a fresh identity and current preparation.
