...
- Process spawn requires the authorized source's current Bypass permission, uses its frozen workspace, and accepts an absolute executable plus argv. The default lifetime is the calling scope. Strings written to stdin are UTF-8; decode output incrementally with `TextDecoder`.
...
- `call.http.request` provides source-bound HTTP using Host proxy settings and Bypass permission. Read bounded byte chunks with `response.next()`; `null` means complete, whereas truncation fails. Responses close with the calling scope or retirement. No automatic retries or redirects: remote side-effect recovery belongs to the plugin.
...
- `call.sessions.list({ revision?, cursor? })` returns bounded metadata pages. Agent calls see their own Session; independent calls require `read_sessions` for the selected scope. Catalog access never grants execution authority.
...
- `ctx.preferences.read()` returns revisioned personalization and the workspace-instructions setting, never credentials or full Host configuration. It is available during activation and expires with the plugin; it grants no resource or execution authority.
