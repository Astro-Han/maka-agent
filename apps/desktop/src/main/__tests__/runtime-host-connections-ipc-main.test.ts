...
test('projects exact external provider configuration without inventing legacy connection fields', () => {
  const connection: ConnectionCatalogEntry = {
      ...
      configuration: { region: 'local', project: 'research' },
      enabled: true,
      enabledModelIds: ['model-1', 'model-2'],
      models: [{ id: 'model-1' }, { id: 'model-2' }],
      catalogEntries: [],
  };
  for (const defaultTarget of [null, { connectionId: 'connection-1', modelId: 'model-1' }]) {
    const projected = projectHostConnections({ revision: 7, defaultTarget, connections: [connection] });
    assert.deepEqual(projected, [connection]);
    assert.notEqual(projected[0]?.models, connection.models);
    assert.notEqual(projected[0]?.enabledModelIds, connection.enabledModelIds);
  }
});
