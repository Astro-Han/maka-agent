...
describe('single live-turn handoff', () => {
  it('keeps activity in a single status row before the session or Turn arrives', async () => {
    const session: NonNullable<Parameters<typeof ChatView>[0]['activeSession']> = {
      ...
      llmConnectionSlug: 'conn', connectionLocked: false, model: 'model', sandboxMode: 'workspace-write' as const,
    };
    for (const activeSession of [undefined, session]) {
      ...
      const statuses = document.querySelectorAll('.maka-turn-statusbar [role="status"]');
  ...
  it('renders a transient user message without manufacturing a Turn', async () => {
    const markup = await renderWithLocale(createElement(ChatView, {
      activeSession: {
        ...
        labels: [], isFlagged: false, isArchived: false, hasUnread: false,
        llmConnectionSlug: 'conn', connectionLocked: false, model: 'model', sandboxMode: 'workspace-write',
      },
      messages: [
