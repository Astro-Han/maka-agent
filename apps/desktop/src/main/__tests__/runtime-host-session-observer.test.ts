...
test('reports a failed final catch-up and retries it on the same subscription without another write', async () => {
  const firstEvents = new AsyncFrameQueue();
  const target = eventTarget(1);
  let watermark = 2;
  ...
  const batches: DesktopTranscriptBatch[] = [];
  const transcriptTarget = (consumerId: string): RuntimeHostTranscriptTarget => ({
    id: 7,
    send(_channel, batch) {
      batches.push(batch);
      queueMicrotask(() =>
        observer.acknowledgeTranscript(consumerId, batch.generation, batch.deliverySequence, 7),
      );
    },
    once() {},
    off() {},
  });
  await observer.openTranscript('session-1', 'consumer-1', transcriptTarget('consumer-1'));
  ...
  await waitFor(() => batches.some((batch) => batch.readError?.includes('global cache limit')));
  assert.equal(decodeAttempts, 2, 'a failed read must not spin');

  await observer.closeTranscript('consumer-1', 7);
  failDecode = false;
  await observer.openTranscript('session-1', 'consumer-2', transcriptTarget('consumer-2'));
  await waitFor(() =>
    batches.some((batch) => durableSequences(batch).includes(3)),
  );
  assert.equal(decodeAttempts, 3);
  assert.ok(
...
test('a trailing catch-up failure reaches subscription recovery before the next reader', async () => {
  const firstEvents = new AsyncFrameQueue();
      ...
      openSession: async () => {
        ...
        return runtimeHostSessionFixture({
          ...
          loadTranscriptPage: first
            ...
            : async (input) => transcriptPage(input.direction, input.throughSequence),
          async close() { events.end(); },
        });
      },
    },
  ...
  // The watermark moves between catch-up and settlement. The trailing
  // read's failure must reach the same caller instead of being swallowed.
  watermark = 4;
  firstEvents.push({
    kind: 'subscription.transcript_advanced',
    hostEpoch: 'host-1',
