...
describe('browser logic', () => {
  ...
  it('viewportBoundsAtScale converts CSS pixels to DIP without edge seams', () => {
    ...
    assert.deepEqual(
      viewportBoundsAtScale({ x: 10.25, y: 20.25, width: 301.5, height: 201.5 }, 2),
      { x: 21, y: 41, width: 603, height: 403 },
    );
    assert.equal(viewportBoundsAtScale({ x: 0, y: 0, width: Number.MAX_VALUE, height: 100 }, 2), null);
