...
describe('useTaskEntryController', () => {
  ...
  it('keeps same-id Projects scoped to their owning Runtime Host', async () => {
    ...
    const services = createFakeTaskEntryServices({
      catalog: {
        ...
        getCatalog: async () => ({
          ...
          hosts: [local, remote, { ...remote, profile: { ...remote.profile, id: 'remote-alias' } }],
        }),
    ...
    await act(async () => renderController(root, services));
    assert.equal(controller().selectors.projectScopes.length, 2, 'multiple connections do not duplicate a Host project');
