...
import { isE2e, isIsolatedE2e, revealMode } from "./startup-context.js";
...
import { installDesktopShellPresentation, installDesktopStartupBranding } from "./desktop-shell-presentation.js";
...
export const mainWindowDelegates = {
  ...
  onMainWindowClosed: (): void => { if (process.platform !== "darwin") app.quit(); },
};
...
const resolvedLocalStorageRoot = !isE2e ? undefined : await (e2eFixture
...
if (isE2e && !resolvedLocalStorageRoot) {
  app.quit();
  await new Promise<never>(() => {});
  throw new Error("Desktop storage root resolution did not complete");
}
export const fixtureStorageRoot = resolvedLocalStorageRoot;
...
// A failed document load cannot prevent Host startup and its recovery paths.
const firstPaintBudget = setTimeout(resolveFirstWindowConstructed, 2_000);
void firstWindowConstructed.then(() => clearTimeout(firstPaintBudget));
...
let windowFailurePresented = false;
export const quitCoordinator = createAppQuitCoordinator({
  ...
  onWindowCreationError: (error) => {
    ...
    if (windowFailurePresented) return;
    windowFailurePresented = true;
    if (isIsolatedE2e) { app.exit(1); return; }
    mainWindowController.browserWindow()?.hide();
    void desktopLocale.resolve().then((locale) => showFatalStartupError(error, {
      locale,
      environment: desktopDiagnostics.environment,
      mainLogs: desktopDiagnostics.mainLogs,
      writeClipboard: (report) => clipboard.writeText(report),
      showMessageBox: (options) => showBrowserMessageBox(options, undefined, { locale, revealMode }),
    })).catch((failure: unknown) => {
      console.error("[window] startup diagnostics failed:", failure);
    }).finally(() => app.quit());
  },
  resumeQuit: () => app.quit(),
});
...
installDesktopShellPresentation({ mainWindowController, focusOrCreateWindow: quitCoordinator.focusOrCreateWindow });
