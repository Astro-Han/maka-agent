...
export function createDesktopHostHandoffSurface(input: {
  ipcMain: IpcMain;
  authorize(event: Electron.IpcMainInvokeEvent): boolean;
  ...
  input.ipcMain.handle('runtime-host-handoff:current', (event) => {
    if (!input.authorize(event)) throw new Error('Host handoff requires the main renderer');
    return payloadFor(currentEntry());
  });
  input.ipcMain.handle(
    'runtime-host-handoff:decide',
    (event, payload: { revision?: unknown; action?: unknown }) => {
      if (!input.authorize(event)) throw new Error('Host handoff requires the main renderer');
      if (typeof payload?.revision !== 'string' || typeof payload?.action !== 'string') throw new TypeError('Invalid Host handoff decision');
      ...
      throw new Error('Host handoff decision is no longer available');
    },
  );
