...
export class RuntimeHostSessionObserver {
  ...
  async openTranscript(
    ...
    let admitted = false;
    try {
      await Promise.race([state.subscriptionOwner.ensureTranscriptCurrent(), cancelled]);
  ...
  async readTranscriptTurn(sessionId: string, turnId: string): Promise<StoredMessage[]> {
    ...
    state.pendingTranscriptConsumers += 1;
    try {
      await state.subscriptionOwner.ensureTranscriptCurrent();
  ...
  #state(sessionId: string): ObservedSessionState {
    ...
    const subscriptionOwner = new RuntimeHostSessionSubscriptionOwner({
      ...
      transcriptReadFailed: (error) => {
        for (const consumer of state.transcriptConsumers.values()) {
          consumer.readError = error.message.slice(0, 1024) || 'Transcript read failed';
    ...
    task = (async () => {
      try {
        while (state.transcriptConsumers.get(consumer.consumerId) === consumer) {
          if (consumer.readError) {
            const readError = consumer.readError;
            consumer.readError = undefined;
            const replica = state.replica;
            if (!replica || replica.generation !== consumer.generation) continue;
            await this.#sendTranscriptBatches(consumer, [{
              sessionId: replica.sessionId,
              generation: replica.generation,
              hostEpoch: replica.hostEpoch,
              durableThrough: replica.durableThrough,
              fragments: [],
              reset: false,
              ready: true,
              readError,
      ...
      if (consumer.readError && state.transcriptConsumers.get(consumer.consumerId) === consumer) {
        void this.#scheduleTranscriptDelivery(state, consumer).catch(() => undefined);
      }
    });
