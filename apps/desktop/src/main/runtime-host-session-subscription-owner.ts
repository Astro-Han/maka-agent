...
export interface RuntimeHostSessionSubscriptionOwnerDeps {
  ...
  transcriptReadFailed(error: Error): void;
...
export class RuntimeHostSessionSubscriptionOwner {
   ...
   */
  async ensureTranscriptCurrent(): Promise<void> {
    while (!this.#closed) {
      await this.waitUntilReady();
      const attempt = this.#attempt;
      if (!attempt) return;
      if (!attempt.replica?.resident) {
        await this.#reseedTranscriptReplica();
        continue;
      }
      try {
        await attempt.replica.advance();
      } catch (error) {
  ...
  #failAttempt(
    ...
    if (absorbUnclassified) {
      this.#deps.transcriptReadFailed(failure);
      return false;
    }
    this.#deps.terminalFailure(failure);
    return true;
