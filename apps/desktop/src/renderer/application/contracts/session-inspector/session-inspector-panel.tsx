 ...
 * paragraph.
 */
export function SessionInspectorPanel(props: { sessionId: string; active: boolean; inspector: SessionInspectorService; copy: InspectorCopy; extensions?: ReactNode }) {
  ...
  const overview = useMemo(
    () => deriveInspectorOverviewModel(snapshot.context),
    [snapshot.context],
  );
  ...
  return (
    ...
    >
      ...
      <VStack gap={6} height="100%">
        ...
        {props.extensions}
        {(overview.context || overview.composition) && (
          <InspectorOverview copy={copy} locale={locale} overview={overview} />
        )}
...
function InspectorOverview(props: {
  ...
  return <VStack gap={6} data-maka-contract="session-inspector-overview">
    {overview.context && <InspectorContextSection copy={copy} context={overview.context}
      formatCompactNumber={formatCompactNumber} formatNumber={formatNumber} />}
    {overview.composition && <InspectorCompositionSection copy={copy} state={overview.composition}
