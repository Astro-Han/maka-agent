...
import type { HostHandoffAction } from '@maka/runtime-host/client';
...
export function RuntimeHostHandoffOverlay() {
  ...
  const decide = (action: HostHandoffAction) => {
    setPending(view.revision);
    setFailedRevision(null);
    void handoff.decide(view.revision, action)
      .catch(() => setFailedRevision(view.revision))
      .finally(() => setPending((current) => current === view.revision ? null : current));
  };
      ...
      <Layout
        ...
        content={(
          <LayoutContent padding={4}>
            <span data-maka-content-ready />
            {failedRevision === view.revision ? <p role="alert">{copy.actionFailed}</p> : null}
            {presentation.detail ? (
          ...
          <LayoutFooter>
            {presentation.actions.map(({ action, label }) => (
              <Button
                ...
                label={label}
                isDisabled={pending === view.revision}
