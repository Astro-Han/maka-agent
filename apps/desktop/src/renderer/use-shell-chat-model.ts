...
export function useShellChatModel(options: {
  ...
  // Bind draft thinking to both the task scope and the exact model.
  const [thinkingChoice, setThinkingChoice, clearPendingNewChatThinkingLevel] =
    useNewTaskChoice<{ model: NewChatModel; level: ThinkingLevel | null }>(options.newTaskKey);
  ...
  const pendingNewChatThinkingLevel =
    thinkingChoice && newChatModel &&
    thinkingChoice.model.llmConnectionId === newChatModel.llmConnectionId &&
    thinkingChoice.model.llmConnectionSlug === newChatModel.llmConnectionSlug &&
    thinkingChoice.model.model === newChatModel.model &&
    (thinkingChoice.level === null || newChatModelChoice?.thinkingLevels.includes(thinkingChoice.level))
      ? thinkingChoice.level
      : undefined;
  const setPendingNewChatThinkingLevel = (level: ThinkingLevel | null) => {
    if (newChatModel) setThinkingChoice({ model: newChatModel, level });
  };
