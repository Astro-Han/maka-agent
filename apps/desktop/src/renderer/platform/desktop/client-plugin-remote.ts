  ...
  return (identity: ClientIdentity, signal: AbortSignal) => {
    ...
    const close = (): Promise<void> => {
      ...
      closing = (async () => {
        if (!document) return;
        let id: string;
        try { id = await document; } catch { return; }
    ...
    const retired = () => { void close().catch(() => {}); };
    ...
    const bind = (name: string, sessionId: string | undefined, handler: 'method' | 'stream') => {
      const binding: PluginRemoteBinding = { client: identity, method: name, sessionId: sessionId ?? null };
      let target: Promise<PluginRemoteTarget> | undefined;
      return async () => {
        assertLive();
        target ??= request({ kind: 'bind', binding }).then((result) => {
          if (result.kind !== 'bound' || result.handler !== handler) throw new Error('Remote handler kind mismatch');
        ...
        return async function* (input: I, cancellation?: AbortSignal): AsyncGenerator<O> {
          ...
          try {
            for (;;) {
              ...
              if (result.kind === 'end') { ended = true; return; }
