...
import { verifySystemPrompt } from './client-system-prompt-workflow.mjs';

const { values } = parseArgs({
  options: {
    socket: { type: 'string' },
    'root-id': { type: 'string' },
    'system-prompt-workspace': { type: 'string' },
    reopened: { type: 'boolean' },
  },
});
assert.equal(RUNTIME_HOST_COMPATIBILITY_EPOCH, 152);
const clients = [];
...
try {
  ...
  await verifySystemPrompt(
    connection,
    values['system-prompt-workspace'],
    values.reopened,
    connectClient,
  );
  console.log(values.reopened ? 'system-prompt-reopened' : 'system-prompt-passed');
} finally {
