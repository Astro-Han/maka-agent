...
export async function verifyMessageSubmit(connection, workspace, reopened, openClient) {
  ...
  const submit = (input, client = connection) => client.request('turn.message.submit', input, 3000);
  ...
  if (reopened) {
    ...
    assert.deepEqual(
      await request('skill.catalog.query', previous.sourceQuery),
  ...
  await verifyInvocableCatalog(request, workspace);
  const sourceContext = { workspace: { kind: 'host_path', path: workspace } };
  const sourcePage = (view) =>
    request('skill.catalog.query', { kind: 'start', context: sourceContext, view });
  ...
  await writeFile(join(alias, 'skill.lock.json'), JSON.stringify(lock));
  const invalidated = await request('skill.catalog.query', {
    kind: 'continue',
    context: sourceContext,
  ...
  const sourceChanged = await request('skill.catalog.query', {
    kind: 'continue',
    context: sourceContext,
    view: 'managed_sources',
  ...
  try {
    ...
    const skillTarget = { kind: 'session', sessionId };
    const skillPage = await request('skill.catalog.invocable.query', {
      kind: 'start',
      target: skillTarget,
    });
    assert.deepEqual(
    ...
    const blocked = await submit({
      ...
      skillIds: ['project:maka:disabled'],
    });
    assert.equal(blocked.disposition, 'blocked');
    assert.equal(blocked.skillInvocation.failed[0].reason, 'disabled');
    ...
    assert.deepEqual(result.skillInvocation.loaded, [{ id: 'review', name: 'Review' }]);
    ...
    await model.partial;
    assert.deepEqual(await submit(second), active, 'active exact retry does not start another Run');
    ...
    const changedSkills = await request('skill.catalog.invocable.query', {
      kind: 'continue',
      target: skillTarget,
      revision: skillPage.revision,
      cursor: 'obsolete',
    });
    assert.equal(changedSkills.kind, 'revision_changed');
    const futureSkills = await request('skill.catalog.invocable.query', {
      kind: 'start',
      target: skillTarget,
    });
    assert(
    ...
    assert.equal(incompatible.skillInvocation.failed[0].reason, 'host_incompatible');
    ...
    assert.deepEqual(lateReceipt.skillInvocation.loaded, [{ id: 'review', name: 'Review' }]);
    await writeFile(
    ...
    const legacy = {
      ...
      skillIds: ['project:maka:legacy', 'missing'],
    };
    ...
    assert.equal(legacyBlocked.skillInvocation.loaded.length, 0);
    assert.equal(model.requests.length, 4);
    await assert.rejects(
    ...
    assert.deepEqual(legacyStarted.skillInvocation.loaded, [{ id: 'legacy', name: 'Legacy' }]);
    assert.deepEqual(legacyStarted.skillInvocation.failed, [
    ...
    await assert.rejects(
      request('turn.start', { ...legacy, skillIds: ['legacy'] }),
      (error) => error.code === 'operation_conflict',
    ...
    await assert.rejects(
      request('skill.catalog.invocable.query', { kind: 'start', target: skillTarget }),
...
async function verifyInvocableCatalog(request, workspace) {
  const target = {
    kind: 'new_session',
    context: { workspace: { kind: 'host_path', path: workspace } },
    collaborationMode: 'agent',
    permissionMode: 'ask',
  };
  const query = (input) => request('skill.catalog.invocable.query', { ...input, target });
  ...
  try {
    ...
    await assert.rejects(
      query({ kind: 'continue', revision: first.revision, cursor: 'invalid' }),
    ...
    let page = await query({ kind: 'start' });
    const seen = [];
    let pages = 0;
    while (true) {
      ...
      page = await query({ kind: 'continue', revision: page.revision, cursor: page.nextCursor });
    }
    ...
    const projectTarget = {
      ...target,
      context: { workspace: { kind: 'project', projectId: registered.project.id } },
    };
    const projectPage = await request('skill.catalog.invocable.query', {
      kind: 'start',
    ...
    await assert.rejects(
      request('skill.catalog.invocable.query', { kind: 'start', target: projectTarget }),
      (error) => error.code === 'operation_unavailable',
    );
    await assert.rejects(
      request('skill.catalog.invocable.query', {
        kind: 'start',
        target: { ...target, collaborationMode: 'plan' },
      }),
      (error) => error.code === 'operation_unavailable',
    );
