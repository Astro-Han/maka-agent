...
async fn account_inventory_initializes_chat_choices_without_overwriting_user_intent() {
    ...
    assert!(matches!(
        ticket.complete("grant".into(), 11).await.unwrap(),
    ...
    let mut ticket = prepare(&store, existing("again", &id)).await;
    ...
    assert!(matches!(
        ticket.complete("new-grant".into(), 13).await.unwrap(),
        LoginCompletion::Committed(_)
    ));
    ...
    store
        ...
        .unwrap();
    assert!(matches!(
        ticket.complete("other-grant".into(), 15).await.unwrap(),
        LoginCompletion::Committed(_)
    ));
