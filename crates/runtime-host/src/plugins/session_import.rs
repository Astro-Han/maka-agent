...
use maka_session_import::plugin::{Builtin, ID};
use std::sync::Arc;
...
pub(crate) fn install(setup: &mut Setup) -> Result<(), maka_plugins::Error> {
    if setup.builtins.contains_key(ID) || setup.layers.contains_key(ID) {
        return Err(maka_plugins::Error::Invalid(
            "built-in Session import identity is reserved".into(),
        ));
    }
        ...
        Arc::new(Definition {
            ...
            plugin: Arc::new(Builtin {
                client: maka_plugins::client::Bundle::builtin(
                    ...
                    include_str!(concat!(env!("OUT_DIR"), "/session-import-client.js")),
                )?,
            }),
        }),
    );
    ...
    let mut client = Entry::new("maka.session-import.ui")?;
    client.package_id = Some(ID.into());
    client.inject = maka_plugins::composition::Injection::Names(vec![
        maka_session_import::plugin::client_service(ID),
    ]);
    setup.layers.insert(
        ID.into(),
        vec![
