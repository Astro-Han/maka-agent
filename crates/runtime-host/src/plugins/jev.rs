...
use super::Setup;
use maka_jev::plugin::{Builtin, ID};
use maka_plugins::{
...
pub(crate) fn install(setup: &mut Setup) -> Result<(), maka_plugins::Error> {
        ...
        Arc::new(Definition {
            ...
            plugin: Arc::new(Builtin {
                client: Some(maka_plugins::client::Bundle::builtin(
                    ...
                    include_str!(concat!(env!("OUT_DIR"), "/jev-client.js")),
                )?),
            }),
        }),
    );
    ...
    let mut client = Entry::new("maka.jev.ui")?;
    client.package_id = Some(ID.into());
    client.inject =
        maka_plugins::composition::Injection::Names(vec![maka_jev::plugin::client_service(ID)]);
    setup.layers.insert(
        ID.into(),
        vec![
            Operation::Insert {
