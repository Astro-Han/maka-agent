...
use super::Setup;
use maka_goal::plugin::{Builtin, ID};
use maka_plugins::{
...
pub(crate) fn install(setup: &mut Setup) -> Result<(), maka_plugins::Error> {
    if setup.builtins.contains_key(ID) || setup.layers.contains_key(ID) {
        return Err(maka_plugins::Error::Invalid(
            "built-in Goal identity is reserved".into(),
        ));
    }
        ...
        Arc::new(Definition {
            ...
            plugin: Arc::new(Builtin {
                client: Some(maka_plugins::client::Bundle::builtin(
                    ...
                    include_str!(concat!(env!("OUT_DIR"), "/goal-client.js")),
                )?),
            }),
        }),
    );
    ...
    let mut client = Entry::new("maka.goal.ui")?;
    client.package_id = Some(ID.into());
    client.inject =
        maka_plugins::composition::Injection::Names(vec![maka_goal::plugin::client_service(ID)]);
    setup.layers.insert(
        ID.into(),
        vec![
            Operation::Insert {
