...
pub(super) fn register(
    ...
    PluginTool::new(ToolRegistration {
        definition: ToolDefinition {
            name: NAME.into(),
            description: "Create and manage scheduled tasks (定时任务) owned by this conversation. All tasks also appear in Desktop. session_resume (default) continues this conversation; agent_run uses a frozen model/workspace/policy template for a fresh session; notify_local sends a local notification. Times are Unix milliseconds; omitted startAt means now, cron uses the configured IANA timezone. Missed unclaimed triggers are skipped by default. list returns compact summaries; management cannot change another conversation's tasks.".into(),
            ...
            Ok(PreparedEffect::new(move |cancel| {
                Box::pin(async move {
                    ...
                    if matches!(input, Input::List {}) {
                        ...
                        &task.created_by, maka_scheduler::task::Creator::Agent { session_id } if *session_id == context.invocation.session_id
                    )).map(|task| json!({
                    ...
                    let result = service
                        .mutate(
                            mutation,
                            maka_scheduler::authorization::Origin::Agent(context.invocation),
                        )
                        .await
                        .map_err(failed)?;
                    let output = match result {
                        maka_scheduler::command::MutationResult::Task { task } => json!({
                            "id":task.id,"title":task.title,"status":task.status,"nextFireAt":task.next_fire_at,
                        }),
                        maka_scheduler::command::MutationResult::Deleted { task_id } => {
