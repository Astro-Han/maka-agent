...
use maka_graph::{decision::Decision, owner::Handle, schedule::Source};
...
pub(super) fn register(
    handle: Handle,
    operators: Arc<super::NativeOperators>,
) -> Result<Staged, String> {
    let swarm = handle.snapshot().swarm.is_some();
    let handler = Arc::new(GraphTools(handle, operators));
    ...
    for (name, description, input_schema, semantics) in [
        ...
        if name == "agent_swarm_status" && !swarm {
            continue;
        }
...
#[serde(deny_unknown_fields)]
struct StatusInput {
    after: Option<maka_graph::WorkId>,
}
#[derive(serde::Deserialize)]
#[serde(deny_unknown_fields)]
struct ResultQuery {
    ...
    graph_id: Option<maka_graph::GraphId>,
    work_id: maka_graph::WorkId,
    record_id: String,
    #[serde(default)]
    offset: usize,
}
