...
pub(super) async fn execute(
        ...
        Input::Update(i) => {
            ...
            if i.text.contains("/skill:") {
                let active_tools = (source.disposition
                    ...
                    .flatten();
                let skills = if let Some(tools) = active_tools {
                    std::sync::Arc::new(
                        host.executions
                            .load_skills(
                                &observation.session.configuration.workspace.host_cwd,
                                tools.as_ref().clone(),
                            )
                            .await?,
                    )
                    ...
                    host.executions
                        .prepare_tools(
                            session,
                            &observation.session.configuration,
                            Some(connection_id),
    ...
    let change = host
        ...
        .await
        .map_err(|e| storage_error(host, e))?;
    Ok(output(&host.epoch, operation, change))
}
