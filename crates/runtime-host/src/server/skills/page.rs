...
use super::{FrozenSkills, failure};
use maka_protocol::{OperationError, OperationErrorCode as Code, skills::*};
use serde::Serialize;
use std::collections::BTreeSet;

pub(super) fn invocable(
    ...
    let preferences = match &skills.preferences {
        maka_skills::Preferences::Available(prefs) => prefs,
        maka_skills::Preferences::Unavailable => {
            return Err(failure(
                Code::PersistenceFailed,
                "Skill preferences are unavailable",
            ));
        }
    };
            ...
            || encode(&envelope)?.len() + encoded_size + item_size + commas > MAX_PAGE_BYTES
        {
            if selected.is_empty() {
                return Err(failure(
                    ...
                    "Skill metadata cannot fit a page",
                ));
            }
            break;
        }
...
pub(super) fn decode_cursor(value: &str, revision: &str) -> Result<usize, OperationError> {
    let invalid = || failure(Code::InvalidRequest, "Invalid Skill catalog cursor");
