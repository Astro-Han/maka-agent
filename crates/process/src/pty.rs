...
//! Native PTY transport. The runtime owns admission, persistence and drain.
#[cfg(unix)]
mod stream;
#[cfg(unix)]
mod unix;
...
#[cfg(windows)]
pub use windows::{PtyChild, PtyIo, spawn};
...
/// Fully captured launch data shared by native Unix and ConPTY. It deliberately
...
#[cfg(windows)]
impl Argument {
    fn command_line(&self) -> std::io::Result<String> {
        let value = self
            .as_ref()
            .to_str()
