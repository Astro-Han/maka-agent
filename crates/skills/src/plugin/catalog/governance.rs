...
use super::bounded;
use crate::api::*;
use crate::plugin::PreferenceSnapshot as SkillPreferences;
use crate::{DiscoveryFailure, OriginFailure, OriginStatus, SkillLocation, SourceCatalog};
use maka_runtime::artifact::content_digest;
use std::collections::BTreeMap;

/// No Run context is attached to this query: eligibility is not advertisement.
pub(in crate::plugin) fn items(
...
fn row(location: &SkillLocation, preferences: Option<&SkillPreferences>) -> GovernanceItem {
    ...
    GovernanceItem {
        ...
        shadowed_by: None,
        needs_review: false,
        manageable: (location.scope == SkillScope::Workspace
            && location.source == SkillSource::Legacy
            || location.scope == SkillScope::User
                && matches!(location.source, SkillSource::Maka | SkillSource::Agents))
            && crate::safe_source_id(&location.id)
            && preferences.is_some(),
    }
}
...
fn issues(item: &mut GovernanceItem, issues: &[crate::Issue]) {
    for issue in issues {
