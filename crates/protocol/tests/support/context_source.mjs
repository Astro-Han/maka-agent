...
await withSourceModule('packages/runtime-host/src/protocol/context.ts', async (source) => {
  let input = '';
  for await (const chunk of process.stdin) input += chunk;
  for (const [index, test] of JSON.parse(input).entries()) {
    const spec = source.CONTEXT_OPERATION_SPECS[test.operation ?? 'context.compact'];
    let actual;
    try {
      const value =
