...
pub struct Search {
    pub history: Option<Box<super::super::history::History>>,
...
impl Saved {
    ...
    pub(super) fn restore(self) -> Search {
        let mut search = Search {
            history: self.history.then(Box::default),
            ..Search::default()
        };
...
impl Transcript {
    pub fn search_command(&mut self, command: Command) {
        match command {
            ...
            Command::Scope => {
                ...
                search.history = if search.history.is_some() {
                    None
                } else {
                    Some(Box::default())
                };
            ...
            Command::Next | Command::Previous => {
                ...
                if let Some(history) = &mut search.history {
                    history.navigate(command == Command::Next);
                    return;
                }
    ...
    pub(super) fn refresh_search(&mut self, reveal: bool) {
        ...
        if let Some(history) = &mut search.history {
            history.changed(search.editor.text(), self.trace);
            return;
        }
        ...
        match event {
            Event::Key(key) if key.kind != KeyEventKind::Release => {
                ...
                match key.code {
                    ...
                    KeyCode::Char('r')
                        if key.modifiers.contains(KeyModifiers::CONTROL)
                            && search.history.is_some() =>
                    {
                        self.search_command(Command::Restart)
                    }
                    KeyCode::Up | KeyCode::Down if search.history.is_some() => {
                    ...
                    KeyCode::PageUp | KeyCode::PageDown if search.history.is_some() => {
                        if let Some(preview) = search
                            .history
                            .as_mut()
                            .and_then(|history| history.preview.as_mut())
                        {
