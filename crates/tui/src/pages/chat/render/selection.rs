...
mod tests {
    ...
    fn mouse_selection_copies_semantic_unicode_tables_and_links_across_reflow() {
        ...
        let point = position(&view, &key, 0);
        let click = Action::ToggleMessage(key.clone());
        view.text_mouse(
            mouse(MouseEventKind::Down(MouseButton::Left), point),
            Some(click),
        );
        assert!(matches!(
            view.text_mouse(mouse(MouseEventKind::Up(MouseButton::Left), point), None),
            Some(Some(Action::ToggleMessage(_)))
        ));
        view.text_mouse(
            mouse(MouseEventKind::Down(MouseButton::Left), point),
            Some(Action::ToggleMessage(key.clone())),
        );
        view.text_mouse(
            mouse(
    ...
    fn edge_drag_scrolls_on_deadlines_and_stops_at_bounds_release_and_focus_loss() {
        ...
        app.chat.view.search_command(search::Command::Open);
        app.chat.view.search_command(search::Command::Scope);
        app.chat
            .view
            .search
            .as_mut()
