...
impl Transcript {
    pub(super) fn sync_timings(
        ...
        for row in rows.values().filter(|row| row["type"] == "turn_state") {
            ...
            let timing = self.timings.entry(turn.into()).or_default();
            match status {
        ...
        let turns: Vec<_> = self
            ...
            .iter()
            .filter(|(turn, timing)| {
                timing.start.is_some()
                    && (timing.end.is_some() || self.active_turn.as_ref() == Some(turn))
            })
    ...
    pub fn timing_visible(&self) -> bool {
        self.order.iter().zip(&self.starts).any(|(key, start)| {
            key.part == Part::Timing
                && self.active_turn.as_ref() == Some(&key.turn)
                && self
                    .timings
                    .get(&key.turn)
                    .is_some_and(|timing| timing.end.is_none())
