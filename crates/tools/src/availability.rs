...
pub(super) const SEARCH: &str = "tool_search";
...
impl Availability {
    ...
    pub fn enabled(&self) -> bool {
        self.catalog.discovery
            && self
                .catalog
                .definitions()
                .any(|definition| !direct(&definition.name))
    }
    ...
    pub fn snapshot(&self) -> ToolCatalog {
        ...
        self.catalog
            .select(|name| direct(name) || active.contains(name))
    }
    pub fn clear(&self) {
    ...
    fn search(&self, input: &SearchInput) -> SearchResult {
        ...
        let mut ranked: Vec<_> = self
            .catalog
            .definitions()
            .filter(|d| !direct(&d.name) && !active.contains(&d.name))
            .filter_map(|d| {
