...
| Session lineage | Public Rust/JS history copying and source reads, native branch/revision create/abandon, retention notices and durable receipts exist. Desktop editing and receipt-based Client recovery remain. Regeneration is removed; edits use revision and resend. | **Host.** One transactional lineage and workspace authority; plugins may request changes through commands, not reproduce them in private storage. |
