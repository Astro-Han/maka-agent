...
| Session lineage | Public Rust/JS history copying, revision abandonment, original input reads, retention notices and durable recovery exist. Native Client routes and Desktop editing remain. Regeneration is removed; edits use revision and resend. | **Host.** One transactional lineage and workspace authority; plugins may request changes through commands, not reproduce them in private storage. |
