...
| **WorkHub:** `maka.workhub` owns coordination prompt and client-tool admission policy; routing, candidates, actions and recovery still live in Host server/execution modules. | Move the remaining workflow, queries, associations, selection, correction/resume policy and UI into the built-in domain. Host commands retain exact-message targeting, atomic admission/cancellation/receipts and settlement. Reuse existing domain records and transactions; do not split atomic correction into independent plugin and Host writes. |
