...
| **WorkHub:** `maka.workhub` owns coordinator configuration, answer composition, native `workhub_tasks`, routing/selection/correction/Stop/Resume workflows and recovery policy. Its Client resolves main and floating-window Session bindings through Remote; backend retirement withdraws the Client and rejects stale calls. Desktop supplies window control and workspace/preferences, not task orchestration. | Migrate the full conversation UI and feedback consumer; replace the private execution profile and complete coordinator steering/followup. Preserve exact-message targeting, atomic admission/cancellation/receipts and Host settlement of accepted work. |
