...
| **WorkHub:** `maka.workhub` owns coordinator configuration, answer composition, native `workhub_tasks`, routing/selection/correction/Stop/Resume workflows and recovery policy. Its Client resolves main and floating-window Session bindings and reads delegated feedback by canonical message lineage through Remote; backend retirement withdraws the Client and rejects stale calls. Desktop supplies window control and workspace/preferences, not task orchestration. | Conversation views, task models and the controller live in `packages/workhub`; the controller accepts explicit Session ports, not native-window services. Finish Client mounting and origin-bound adapters; replace the private execution profile and complete coordinator steering/followup. Preserve exact-message targeting, atomic admission/cancellation/receipts and Host settlement of accepted work. |
