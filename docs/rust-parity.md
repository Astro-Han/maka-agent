...
| **WorkHub:** `maka.workhub` owns prompt/client-tool policy, delegation/correction/selection workflows, Stop/Resume request/results, candidate filtering/references, target/default choices and frozen correction recovery policy. Retirement closes pending selection forms; Host retains canonical outcomes, atomic delivery and accepted settlement. | Move coordinator workflows, native tools, Remote and actual Desktop UI into the built-in domain. Preserve exact-message targeting and atomic admission/cancellation/receipts; never split a correction into independent plugin and Host writes. |
