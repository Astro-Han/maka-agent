...
| **WorkHub:** `maka.workhub` owns prompt/client-tool policy, delegation/correction workflows, Stop/Resume request/results, candidate filtering/references, selection forms/answers, target/default choices and frozen correction recovery policy. Host commands retain atomic delivery and accepted settlement, including while the plugin is disabled. | Move coordinator workflows, canonical-form orchestration, native tools, Remote and actual Desktop UI into the built-in domain. Preserve exact-message targeting and atomic admission/cancellation/receipts; never split a correction into independent plugin and Host writes. |
