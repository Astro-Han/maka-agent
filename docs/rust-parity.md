...
| **WorkHub:** `maka.workhub` owns coordination prompt, client-tool admission policy and Stop request/result handling. A typed Host command retains exact-owner cancellation and durable settlement; routing, candidates, correction/resume and recovery remain to migrate. | Move the remaining workflow, queries, associations, selection, correction/resume policy and UI into the built-in domain. Host commands retain exact-message targeting, atomic admission/cancellation/receipts and settlement. Reuse existing domain records and transactions; do not split atomic correction into independent plugin and Host writes. |
