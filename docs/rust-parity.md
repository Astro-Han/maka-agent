...
| **WorkHub:** `maka.workhub` owns prompt/client-tool policy, ordinary delegation, Stop/Resume request/results, candidate filtering/references, selection forms/answers and target/default choices. Target preparation runs outside admission; Host commands recheck the instance and canonical source before atomic delivery. | Move correction/recovery, coordinator workflows, native tools, Remote and actual Desktop UI into the built-in domain. Host retains exact-message targeting, canonical forms, atomic admission/cancellation/receipts and settlement; never split a correction into independent plugin and Host writes. |
