...
| **WorkHub:** `maka.workhub` owns prompt/client-tool policy, Stop/Resume request/results, candidate filtering/references, selection forms/answers and target/default choices. Host retains exact-owner cancellation, bounded snapshot reads, canonical forms and final admission. Workflow orchestration, correction, recovery and UI remain to migrate. | Move the remaining workflow, queries, associations, selection, correction/resume policy and UI into the built-in domain. Host commands retain exact-message targeting, atomic admission/cancellation/receipts and settlement. Reuse existing domain records and transactions; do not split atomic correction into independent plugin and Host writes. |
