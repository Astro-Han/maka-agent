...
| Session lineage | Public Rust/JS history copying and source reads, native branch/revision create/abandon, retention notices and receipt-based Client recovery exist. Desktop canonical-input editing remains. Regeneration is removed; edits use revision and resend. | **Host.** One transactional lineage and workspace authority; plugins may request changes through commands, not reproduce them in private storage. |
