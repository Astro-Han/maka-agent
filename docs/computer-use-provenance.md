...
cua-driver is a third-party binary, fetched by `scripts/prepare-cua-driver.mjs`
and verified against the digests recorded in the manifest. It is no longer the
executor Maka selects: it is reached only by a caller that names it.
...
what a caller that names nothing now gets, and it is what the desktop host asks
for. It is not signed, so it is still not distributed: its `distributionReady`
is false and `scripts/verify-macos-arm64-dmg.mjs` forbids its path in a packaged
build. A packaged build therefore has no Computer Use executor at all — the
default only takes effect in a development build.
