...
WorkHub 已支持专属会话解析、查询、模型配置、受限 Desktop 工具与附件读取，以及候选发现和向已有或新建会话委派任务。
委派时原子转移附件；运行中目标通过普通持久消息队列接收 steering；交互式目标选择在重试时复用原候选表单与答案。
关联停止与恢复按委派消息追踪精确执行链，并持久化动作重放结果。停止请求和结果是 Session 所有的日志事实；
恢复可在协调 Run 结束后发布结果，不修改已结束的 Run。结构化委派记录保留原始创建选项和目标附件，
候选查询提供最新有效关联；修正及完整 WorkHub Desktop 验收仍未完成。
