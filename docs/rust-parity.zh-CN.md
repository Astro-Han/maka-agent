...
| **WorkHub：**`maka.workhub` 拥有协调提示词、客户端工具准入策略及 Stop 请求／结果处理；类型化 Host 命令保留精确 owner 取消与持久结算。路由、候选、纠正／resume 与恢复仍待迁移。 | 剩余流程、查询、关联、多选、纠正／resume 策略及 UI 迁入内置领域；Host 命令保留精确消息定位、原子准入／取消／回执及结算。复用领域记录与事务，不能把原子纠正拆成插件和 Host 各自写入。 |
