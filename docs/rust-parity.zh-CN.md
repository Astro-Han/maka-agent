...
- **交界：**使用窄而类型化的 Host 服务，一份数据只有一个持久化所有者。内置领域存储可以使用 SQLx，外部插件不能任意操作 Host 表。
...
| **WorkHub：**`maka.workhub` 拥有协调会话配置、回答组合、原生 `workhub_tasks`、路由／选择／纠正／Stop／Resume 流程和恢复策略。Client 通过 Remote 解析主窗口与浮动窗口 Session 绑定；后端退休撤下 Client 并拒绝旧代调用。Desktop 提供窗口控制与工作区／偏好，不再编排任务。 | 迁移完整会话 UI 和反馈消费者，替换私有执行 profile 并补齐协调会话 steering／followup。保留精确消息定位、原子准入／取消／回执，以及 Host 对已接受操作的结算。 |
...
链接的原生插件通过 `Setup::managed_sessions` 声明托管 Session。Host 在接受客户端前持久化
package/scope owner 与创建身份；停用或重启后，普通修改／执行和其他插件的执行授权仍不能绕过
该 owner。这项保留不验证或接管既有 Session 数据。

“完整领域”指单一业务实现和生命周期，不是搬走所有相关类型和表。
规范 Skill 回执和 WorkHub 执行事实可以继续是 runtime 的类型化契约。
...
| 领域 | 剩余功能 | 目标归属／必要边界 |
| --- | --- | --- |
| Plan | 查询、控制、启动，规划产物、审批及转入执行；目前非 Agent collaboration mode 会被拒绝。 | **插件 + Host。** 插件拥有流程与记录，Host 拥有审批证据、授权和 Turn 准入。扩展 Graph 专用的 behavior 选择入口，不为每种流程重建执行引擎。 |
...
| Web | 内置 WebSearch／WebFetch、`web-search.execute`、来源选择／设置、正文提取及明确截断提示。 | **插件 + Host。** Web 插件拥有来源适配和提取，Host 保留网络、代理、凭据和权限边界。现有 invocation HTTP 不等于用户／后台入口已授权，不能伪造 Tool 调用。 |
...
| Runtime policy | shell／web-search／external-agent 消费，普通 named tool profiles。 | **拆分。** shell 启动策略和能力上限留在 Host；Web／外部 agent 设置由对应领域消费。Profile 提供定义，Host 在准入和每步捕获时取能力交集。只保存设置不算完成。 |
