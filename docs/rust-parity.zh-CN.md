...
| Session 迁入迁出 | 公共 Rust／JS 历史导入已支持有界暂存、精确重试、当前权限上限检查和原子发布。bundle 导入／导出，以及 Codex、Claude Code、OpenCode 来源适配／目录／UI 尚未完成。 | **插件 + Host。** adapter 拥有发现与解析；Host 拥有规范材料、身份和发布。导入对话参与历史、分支及压缩，不声称发生了本机执行或用量。原生 bundle 格式仍是 Host 契约。 |
