...
| **WorkHub：**`maka.workhub` 拥有协调提示词与客户端工具准入策略；路由、候选、操作与恢复仍位于 Host server／execution 模块。 | 剩余流程、查询、关联、多选、纠正／resume 策略及 UI 迁入内置领域；Host 命令保留精确消息定位、原子准入／取消／回执及结算。复用领域记录与事务，不能把原子纠正拆成插件和 Host 各自写入。 |
