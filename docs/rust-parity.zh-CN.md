...
| **WorkHub：**`maka.workhub` 拥有提示词／客户端工具策略、委派／纠正／选择流程、Stop／Resume 请求／结果、候选筛选／引用、目标／默认值决策及冻结纠正意图的恢复策略。退休关闭待答选择表单；Host 保留规范结果、原子投递与已接受操作的结算。 | 协调会话流程、原生工具、Remote 及实际 Desktop UI 迁入内置领域。保留精确消息定位与原子准入／取消／回执；不能把纠正拆成插件和 Host 各自写入。 |
