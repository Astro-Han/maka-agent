...
| Web | 内置 WebSearch／WebFetch、`web-search.execute`、来源选择／设置、正文提取及明确截断提示。 | **插件 + Host。** Web 插件拥有来源适配和提取，Host 保留网络、代理、凭据和权限边界。invocation、Remote 与明确授权的后台 HTTP 共用当前权限检查和资源结算。 |
...
3. **缺失业务领域：**完整实现 Web、Recall、todo，再做 Plan／Goal 和研究／复盘；完成外部 adapter、Insights／健康。复用领域边界，不先在 Host 写新业务再搬一次。
