...
| **WorkHub：**`maka.workhub` 拥有协调会话配置、回答组合、原生 `workhub_tasks`、路由／选择／纠正／Stop／Resume 流程和恢复策略。Desktop 只提供窗口控制与所选工作区／偏好，不再编排任务。取消关闭待答选择表单；Host 保留规范结果、原子投递与已接受操作的结算。 | Remote 及实际 Desktop UI 迁入内置领域。保留精确消息定位与原子准入／取消／回执；不能把纠正拆成插件和 Host 各自写入。 |
