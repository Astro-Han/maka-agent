...
| **WorkHub：**`maka.workhub` 拥有提示词／客户端工具策略、Stop／Resume 请求／结果、候选筛选／引用、选择表单／答案及目标／默认值决策。Host 保留精确 owner 取消、有界快照读取、规范表单及最终准入；流程编排、纠正、恢复和 UI 仍待迁移。 | 剩余流程、查询、关联、多选、纠正／resume 策略及 UI 迁入内置领域；Host 命令保留精确消息定位、原子准入／取消／回执及结算。复用领域记录与事务，不能把原子纠正拆成插件和 Host 各自写入。 |
