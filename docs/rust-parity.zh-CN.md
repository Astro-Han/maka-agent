...
| Session 谱系 | 公共 Rust／JS 历史复制及原始输入读取、原生分支／修订创建与放弃、保留通知及持久回执已有。Desktop 编辑与基于回执的 Client 恢复仍待接入。已移除 regenerate，修改请求使用编辑重发。 | **Host。** 谱系及工作区只有一个事务权威；插件可以请求命令，不能在私有存储中重做。 |
